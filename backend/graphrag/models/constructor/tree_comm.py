...
class FastTreeComm:
    def __init__(
        ...
        self.edge_relations = {
            (u, v): data.get("relation", "related_to")
    ...
    def get_triple_embedding(self, node_id):
        ...
        if node_id not in self.semantic_cache:
            ...
            text = (
                ", ".join(triples)
                if triples
                else self.graph.nodes[node_id]["properties"]["name"]
            )
        ...
        if uncached_ids:
            ...
            with torch.no_grad():
                embeddings = self.model.encode(
                    texts, convert_to_tensor=True, batch_size=128
                )
    ...
    def _compute_jaccard_matrix_vectorized(self, level_nodes):
        ...
        node_to_idx = {node: i for i, node in enumerate(self.node_list)}
        level_indices = [
            node_to_idx[node] for node in level_nodes if node in node_to_idx
        ]
    ...
    def _compute_sim_matrix(self, level_nodes):
        ...
        sim_matrix = (
            self.struct_weight * structural_sim_matrix
    ...
    def detect_communities(
        self, level_nodes, max_iter=1, merge_threshold=0.5, max_total_communities=None
    ):
        ...
        sorted_clusters = sorted(
            initial_clusters.items(), key=lambda x: len(x[1]), reverse=True
        )
        ...
        for cluster_id, cluster_nodes in sorted_clusters:
            ...
            if len(cluster_nodes) <= 3:
                ...
                if len(final_communities) >= max_total_communities:
                    ...
                    sub_communities = self._refine_cluster(
                        cluster_nodes, max_iter, merge_threshold
                    )
        ...
        logger.info(
            f"Generated {len(final_communities)} communities from {len(level_nodes)} nodes"
        )
    ...
    def _refine_cluster(self, cluster_nodes, max_iter, merge_threshold):
        ...
        for iteration in range(max_iter):
            ...
            for sim_info in cluster_similarities:
                ...
                if (
                    cluster1_id not in merged_clusters
                    and cluster2_id not in merged_clusters
                ):
    ...
    def create_super_nodes(
        self, comm_to_nodes: Dict[str, List[str]], level: int = 4, batch_size: int = 5
    ):
        super_nodes = {}
        communities = [
            (comm_id, members)
        ...
        for i in range(0, len(communities), batch_size):
            ...
            for comm_id, members in batch:
                try:
                    ...
                    logger.error(
                        f"Error creating super node for community {comm_id}: {e}"
                    )

        logger.info(f"Created {len(super_nodes)} super nodes")
        return super_nodes

    def extract_keywords_from_community(
        self, community_nodes: List[str], top_k: int = 5
    ) -> List[str]:
        if len(community_nodes) <= top_k:
            return community_nodes

        structural_scores = {
            node: self.degree_cache.get(node, 0) for node in community_nodes
        }
        ...
        combined_scores = {
            node: (
                self.struct_weight * norm_structural[node]
        ...
        top_nodes = sorted(
            community_nodes, key=lambda x: combined_scores[x], reverse=True
        )[:top_k]
        return top_nodes

    def create_super_nodes_with_keywords(
        self, comm_to_nodes: Dict[str, List[str]], level: int = 4, batch_size: int = 5
    ):
        ...
        for comm_id, members in comm_to_nodes.items():
            ...
            try:
                ...
                for keyword in keywords:
                    ...
                    self.graph.add_edge(
                        keyword, keyword_node_id, relation="represented_by"
                    )
                    self.graph.add_edge(
                        keyword_node_id, super_node_id, relation="keyword_of"
                    )

                    for member in members:
                        if member == keyword:
                            self.graph.add_edge(
                                member, keyword_node_id, relation="kw_filter_by"
                            )
