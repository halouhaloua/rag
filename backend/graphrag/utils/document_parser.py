...
try:
    ...
    antiword_check = subprocess.run(
        ["which", "/usr/local/bin/antiword"], capture_output=True
    )
    if antiword_check.returncode == 0:
        ...
        antiword_check = subprocess.run(["which", "antiword"], capture_output=True)
...
class DocumentParser:
    ...
    def _parse_docx(self, docx_path: str) -> Optional[str]:
        ...
        if file_ext == ".doc":
            ...
            if ANTIWORD_AVAILABLE:
                ...
                if result:
                    logger.info(
                        f"Successfully extracted {len(result)} chars from DOC via antiword"
                    )
                    return result
            ...
            if TIKA_AVAILABLE:
                ...
                if result:
                    logger.info(
                        f"Successfully extracted {len(result)} chars from DOC via Apache Tika"
                    )
                    return result
            ...
            if TEXTRACT_AVAILABLE:
                ...
                if result:
                    logger.info(
                        f"Successfully extracted {len(result)} chars from DOC via textract"
                    )
                    return result
            ...
            if result:
                logger.info(
                    f"Successfully extracted {len(result)} chars from DOC via LibreOffice"
                )
                return result
            else:
        ...
        try:
            ...
            file_info = subprocess.run(
                ["file", docx_path], capture_output=True, text=True
            )
        ...
        if is_corrupted:
            logger.warning(
                "⚠️  This WPS Office document cannot be parsed by available tools"
            )
            ...
            logger.info(
                "  3. Use online converters (e.g., zamzar.com, cloudconvert.com)"
            )
        else:
            logger.info("Recommended solutions:")
            logger.info(
                "  1. Install LibreOffice (best compatibility): sudo yum install libreoffice-headless"
            )
            logger.info("  2. Convert file to .docx format using Microsoft Word or WPS")
            logger.info(
                "  3. For standard .doc files: Install antiword (sudo apt-get install antiword)"
            )
        return None
    ...
    def _parse_with_python_docx(self, docx_path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"Successfully extracted {len(extracted_text)} chars via python-docx"
            )
            return extracted_text
    ...
    def _parse_with_antiword(self, doc_path: str) -> Optional[str]:
        ...
        try:
            result = subprocess.run(
                [ANTIWORD_PATH, doc_path], capture_output=True, text=True, timeout=30
            )
    ...
    def _parse_doc_with_libreoffice(self, doc_path: str) -> Optional[str]:
        ...
        try:
            ...
            lo_check = subprocess.run(["which", "libreoffice"], capture_output=True)
            ...
            try:
                ...
                if os.path.exists(txt_path):
                    ...
                    if "no export filter" in result.stderr or "Error:" in result.stderr:
                        logger.warning(
                            f"LibreOffice cannot parse this document format. Error: {result.stderr[:150]}"
                        )
                    return None
            finally:
                # Cleanup
    ...
    def _parse_rtf(self, rtf_path: str) -> Optional[str]:
        ...
        if STRIPRTF_AVAILABLE:
            try:
                ...
                if text and text.strip():
                    logger.info(
                        f"Successfully extracted {len(text)} chars from RTF via striprtf"
                    )
                    return text.strip()
            except Exception as e:
        ...
        if TEXTRACT_AVAILABLE:
            try:
                ...
                if text and text.strip():
                    logger.info(
                        f"Successfully extracted {len(text)} chars from RTF via textract"
                    )
                    return text.strip()
            except Exception as e:
            ...
            if lo_check.returncode == 0:
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        if os.path.exists(txt_path):
                            with open(
                                txt_path, "r", encoding="utf-8", errors="ignore"
                            ) as f:
                                text = f.read()
                            if text.strip():
                                logger.info(
                                    f"Successfully extracted {len(text)} chars from RTF via LibreOffice"
                                )
                                return text.strip()
                finally:
                    if os.path.exists(temp_dir):
